...
public class Boss1_Controller : MonoBehaviour, I_Damage, IBattleResultConsumer
{
    IBattleResultSink battleResultSink;

    public void BindBattleResultSink(IBattleResultSink sink)
    {
        battleResultSink = sink ?? throw new System.ArgumentNullException(nameof(sink));
    }
    ...
    void Start()
    {
        boss1Anim = GetComponent<Boss1Anim>();
    ...
    void Update()
    {
        if (attackCycle == null || battleHealth == null || battleHealth.Result != BattleResult.InProgress)
        {
            return;
        }
    ...
    private void AnimEvent(TrackEntry trackEntry, Spine.Event e)
    {
        ...
        {
            battleHealth.IsDamageable = false;
            OffAllHPColor();
            //进入无敌
        }
    }
    ...
    private void Entry_Complete(TrackEntry trackEntry)
    {
        ...
        AnimPlay(animName_idle, true);
        OnHPColor();
        battleHealth.IsDamageable = true;
    ...
    void CreateBossHP()
    {
        weaknessQueue = WeaknessQueue.CreateBoss1(new UnityRandomSource());
    ...
    public void OnDamage(GameObject _attacker, Weapon atkWapon, Vector2 cPoint, string parm)
    {
        ...
        if (resolution == BossDamageResolution.Victory)
        {
            ...
            battleResultSink?.Show(BattlePresentationResult.Boss1Defeated);
        }
    }
    ...
    void OffAllHPColor()
    {
        SetHPColor(ColorType.hong,0.2f);
        SetHPColor(ColorType.zi, 0.2f);
        SetHPColor(ColorType.lv, 0.2f);
    }

    void SetHPColor(ColorType color,float c)
    {
        switch (color)
        {
            case ColorType.hong:
