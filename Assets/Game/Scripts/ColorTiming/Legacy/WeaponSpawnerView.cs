...
/// </summary>
public abstract class WeaponSpawnerView : MonoBehaviour, ITransientEntityConsumer
{
    ...
    protected abstract bool HasActiveBoss { get; }
    protected abstract bool TryGetCurrentWeakness(out WeaponColor weakness);
    protected abstract WeaponSpawnPolicy CreatePolicy(int activeLimit);
    protected abstract void SubscribeBossDamage();
    protected abstract void UnsubscribeBossDamage();
    ...
    {
        UnsubscribeBossDamage();
    }

    protected virtual void Update()
    {
        if (runtime == null || !HasActiveBoss || weaponT == null || weaponItem == null)
        {
            return;
        }
    ...
    protected void OnBossDamaged()
    {
        ...
        {
            child.GetComponent<Pickup_Weapon>()?.HideTip();
        }
    ...
    public void CreateWeapon_dis(Vector3 position, ColorType color, WeaponType type)
    {
        CreateWeapon(WeaponIdentity.FromLegacy((int)color, (int)type), position);
    }
    ...
    private void CollectActiveColors()
    {
        ...
        foreach (Transform child in weaponT)
        {
            var pickup = child.GetComponent<Pickup_Weapon>();
            if (pickup != null && pickup.Weapon != null)
            {
                activeColors.Add(pickup.Weapon.Identity.Color);
            }
        }
    }
    ...
    private void CreateWeapon(WeaponIdentity identity, Vector3 position)
    {
        ...
        transientEntities.Spawn(
            ...
            instance =>
            {
                var pickup = instance.GetComponent<Pickup_Weapon>();
                if (pickup == null)
                {
                    Debug.LogError($"{name}: weapon prefab is missing Pickup_Weapon.", weaponItem);
                    return;
                }

                pickup.InitPickWeapon(new Weapon(identity));
            });
    }
    ...
    private void CheckWeaponTip()
    {
        ...
        {
            var pickup = child.GetComponent<Pickup_Weapon>();
            if (pickup?.Weapon != null && pickup.Weapon.Identity.Color == weakness)
            {
