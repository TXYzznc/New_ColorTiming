...
namespace ColorTiming.Presentation.Audio
{
    ...
    public sealed class GfColorTimingSoundService : MonoBehaviour, IColorTimingSoundService
    {
        ...
        {
            if (clip == null || GF.Sound == null)
            {
                return 0;
            }

            var relativeName = BuildRelativeName(clip.name, channel);
            var serialId = GF.Sound.PlaySound(
            ...
            if (serialId > 0)
            {
                ...
                if (IsGameplay(channel))
                {
                    ...
                    if (gameplayPaused)
                    {
                        ApplyToGameplaySound(serialId, GF.Sound.PauseSound);
                    }
                }
            }
            return serialId;
        ...
        private void Update()
        {
            ...
            if (!gameplayPaused || GF.Sound == null) return;
            ApplyToGameplaySounds(GF.Sound.PauseSound);
        }
        ...
        void OnScaleChanged(float scale)
        {
            ...
            if (GF.Sound == null) return;
            ApplyToGameplaySounds(gameplayPaused ? GF.Sound.PauseSound : GF.Sound.ResumeSound);
        }
        ...
        void ApplyToGameplaySound(int serialId, Action<int> operation)
        {
            ...
            catch (GameFrameworkException)
            {
                ...
                if (GF.Sound == null || !GF.Sound.IsLoadingSound(serialId))
                {
