...
- FSR 源码以 `LocalPackages/FastScriptReload` Git submodule 固定在框架维护的 fork `TXYzznc/FastScriptReload` 提交 `5ef1a8b8566cc8c18326ef80300fc55bad81062c`。该提交使用官方 `Lib.Harmony.Thin 2.4.2` 及固定依赖，并使 2021+ Roslyn 文件名与程序集名一致，以兼容 Unity 2022.3/Burst。版本、来源、许可证和 SHA-256 见 `LocalPackages/FastScriptReload/Assets/Plugins/Harmony/HarmonyThinDependencies.md`。克隆项目时使用 `git clone --recurse-submodules`；已克隆的项目执行 `git submodule sync --recursive` 后再执行 `git submodule update --init --recursive`。
