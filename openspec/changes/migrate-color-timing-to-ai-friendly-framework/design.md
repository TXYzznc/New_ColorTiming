...
只在以下证据成立时移除脚本：类为空或仅为测试、没有场景/Prefab/Animator/代码引用、且追踪矩阵记录搜索证据。明确缺陷必须同时具备源代码证据、预期不变量和回归测试，例如 Boss2 橙色生命段使用错误索引。所有修复单列在 `Documentation/Refactor/behavior-fixes.md`，不得夹带玩法调整。
