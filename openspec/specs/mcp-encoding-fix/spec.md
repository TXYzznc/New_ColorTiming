# mcp-encoding-fix Specification

## Purpose
TBD - created by archiving change 14-mcp-encoding-fix. Update Purpose after archive.
## Requirements
...
修复过程 MUST 仅修改编码入口、对应 SKILL 指引和测试文件。MUST NOT 触碰项目资产。验收脚本需要写入 Unity 资产时 MUST 使用任务创建的临时副本目录，完成后 MUST 删除副本。
